...
us.personal <- data.frame(USPersonalExpenditure, stringAsFactors = FALSE)

# What are the column names of your dataframe?
colnames(us.personal)
...
rownames(us.personal)

# Create a column "category" that is equal to your rownames
us.personal$category <- rownames(us.personal)
View(us.personal)
# How much money was spent on personal care in 1940?
us.personal["Personal Care", "X1940"]

# How much money was spent on Food and Tobacco in 1960?
us.personal["Food and Tobacco", "X1960"]

# What was the highest expenditure category in 1960?
# Hint: use the `max()` function to find the largest, then compare that to the column
us.personal[us.personal$X1960 == max(us.personal$X1960), "category"]

# Define a function `DetectHighest` that takes in a year as a parameter, and
# returns the highest spending category of that year
DetectHighest <- function(year) {
  high <- us.personal[us.personal$category == max(us.personal$year, "category")]
  return (high)
}

# Using your function, determine the highest spending category of each year
paste(DetectHighest(X1940))
